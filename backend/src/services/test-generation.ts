...
const demoSourceFiles = ["demo-app/payment.html", "demo-app/checkout.spec.ts"]
...
function demoTests(analysis: ChangeAnalysis, diff: string): GeneratedTest[] {
  ...
  const isDemoFixture = sourceFiles.includes("demo-app/payment.html")
...
`,
  ...
  if (types.includes("ui")) {
    tests.push({
      ...
      generatedAt: now,
      sourceFiles,
      content: isDemoFixture
        ? `import { expect, test } from '@playwright/test';

test.describe('checkout payment flow generated by ReleaseGuard AI', () => {
  test('uses the self-healed payment button locator', async ({ page }) => {
    await page.goto('file://' + process.cwd() + '/demo-app/payment.html');

    await expect(page.locator('[data-testid="complete-payment"]')).toBeVisible();
...
${diff}`,
    ...
    logger.warn("Gemini test generation failed - falling back to deterministic demo tests", {
